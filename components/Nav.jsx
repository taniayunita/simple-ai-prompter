  ...
  return (
    <nav className="flex-between w-full mb-16 pt-3">
      ...
      <div className="sm:flex hidden">
        {session?.user ? (
          <div className="flex gap-3 md:gap-5">
            ...
            <Link href="/profile">
              <Image src={ImageProfile} width={37} height={37} className="rounded-full" alt="profile" />
            </Link>
          </div>
        ) : (
          <>
      ...
      <div className="sm:hidden flex relative">
        {session?.user ? (
          <div className="flex">
            <Image src={ImageProfile} width={37} height={37} className="rounded-full" alt="profile" onClick={() => setToggleDropDown((prev) => !prev)} />
            {toggleDropDown && (
